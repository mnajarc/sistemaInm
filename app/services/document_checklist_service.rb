class DocumentChecklistService
  ...
  def documents_by_co_owner
    co_owners = @transaction.business_transaction_co_owners.where(active: true).order(:id)

    return [] if co_owners.empty?

    co_owners.map do |co_owner|
      # ✅ USAR is_primary PARA IDENTIFICAR
      is_primary = co_owner.is_primary
      
      # El principal recibe documentos de ambos tipos
      if is_primary
        submissions = @transaction.document_submissions
...

      {
        co_owner: {
          ...
          is_primary: is_primary  # ✅ AHORA VIENE DEL FLAG
        },
        documents: {
          total: submissions.count,
          uploaded: submissions.select(&:uploaded?).count,
  ...
  def documents_by_co_owner_anterior
